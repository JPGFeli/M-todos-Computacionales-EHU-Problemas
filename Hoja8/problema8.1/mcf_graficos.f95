...
!
!Un vector cuyos valores indican si la correspondiente grafica se realiza con marcas discretas (+) (1), 
!lineas (2), marcas y lineas (3), barras de error (4) o puntos (5). Por defecto la grafica se realiza con marcas.
!
...
!
!Min_x (max_x) fija el valor de x minimo (maximo) para realizar graficos bidimensionales en la misma escala.
!Se requieren ambos valores (no hay valores por defecto).
!
!Min_value (max_value) fija el valor de y o z minimos (maximos) para realizar dibujos en la 
!misma escala. Se requieren ambos valores (no hay valores por defecto).
!
...
!
!Ratio es un parametro opcional con valores 0, 1, -1. Un valor de 1 hace la longitud de los ejes x,y iguales
!independientemente se su longitud. Un valor de -1 utiliza la misma unidad de medida en ambos ejes. Por defecto
!su valor es 0.
!
...
character(len=*),parameter,dimension(1:5) :: plot_type= (/" points     "," lines      ", &
...
real(kind=sp),dimension(1:3) :: test
integer                      :: i,j,digits,count_digits,dim,file_count,x_column,y_column,z_column
...
!
write (unit=666,fmt="(a)") "set terminal wxt enhanced position 1,1 noraise"
!write (unit=666,fmt="(a)") "set terminal x11" 
!*************************************************************************************************

if (present(ratio).and.abs(ratio)==1) then
        write (unit=666,fmt="(a,i0)") "set size ratio ",ratio
end if
...
!plot type: 1 (points), 2 (lines), 3 (linespoints), 4 (errorbars), 5 (dots)

        if (present(type).and.type(i)<=5.and.type(i)>=1) then
                 plot_typ=type(i)
        end if
		     ...
		     if (plot_typ == 4) then
				...
				with = " with"
                 ...
                 if (file_count == 1) then
                        ...
                        if (present(min_x).and.present(max_x).and. &
                            present(min_value).and.present(max_value)) then
                                write(unit=666,advance="no", &
                                      fmt="(a,f0.3,a,f0.3,a,f0.3,a,f0.3,3a,i0,a,i0,2a)") &
                                ...
                                write(unit=666,advance="no", &
                                      fmt="(a,f0.3,a,f0.3,3a,i0,a,i0,2a)") &
                        ...
                        write(unit=666,advance="no",fmt="(3a,i0,a,i0,2a)") &
...
!
!                                       write(unit=666,fmt="(a,i0,a,i0)") "set dgrid ",grid,",",grid
                                        write(unit=666,fmt="(a,i0,a,i0)") "set dgrid3d ",grid,",",grid
                                        write(unit=666,fmt="(a)") "set hidden3d"
                                end if
                                write(unit=666,fmt="(a,i0)") "set cntrparam levels ",levels
...
!                               write(unit=666,fmt="(a)") "set noclabel"
                        ...
                        if (present(min_value).and.present(max_value)) then
                                write(unit=666,advance="no", &
                                      fmt="(a,f0.3,a,f0.3,3a,i0,a,i0,a,i0,2a)") &
                                ...
                                write(unit=666,advance="no", &
                                      fmt="(3a,i0,a,i0,a,i0,2a)") &
                                      "splot """, trim(file(i)),""" using ",x_column, &
                                      ":",y_column, ":",z_column," with ", &
                                      plot_type(plot_typ)
                        end if
                 else
                        write(unit=666,advance="no",fmt="(3a,i0,a,i0,a,i0,2a)") &
...
!               write(unit=666,fmt="(3a)")         "set terminal ",trim(output_type(output_typ))," enhanced color"
                write(unit=666,fmt="(2a)")         "set terminal ",trim(output_type(output_typ))
...
!*************************************************************************************************
! Windows : use the next instruction before version 5.0
!       call system ("pgnuplot ""gnuplot.gnu""")
        call system ("gnuplot ""gnuplot.gnu""")
        call wait(0.3*slides)
end if
